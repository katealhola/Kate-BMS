...
//  Serial.println("SDA:"+String(Wire.sda)+" SCL:"+String(Wire.scl)+" "+String((int32_t)Wire.i2c,16)+" "+String(Wire.num));
  Serial.print("I2C_SCL="+String(I2C_SCL)+" I2C_SDA:"+String(I2C_SDA));
//  i2cscan();
  // Wire.setClock(400000);
  // display.flipScreenVertically();
 //  pinMode(OZ890_RESETN , OUTPUT);
 ...
 //   Wire.begin(I2C_SDA,I2C_SCL,300000);
  ...
  // Wire.setClock(300000); 
   Serial.println();
   Serial.print("StartUp I2C_SCL="+String(I2C_SCL)+" "+String(SCL)+" I2C_SDA:"+String(I2C_SDA)+" "+String(SDA));
...
#ifdef TFT
  xTaskCreate(spiDispkayTask,       /* Task function. */
...
void i2cTask( void * parameter ){
    ...
    while(1) {
      ...
      if(Bms.clearlog) {
        ...
        Bms.logFile = SPIFFS.open(LOGFILE, FILE_APPEND);
        if(!Bms.logFile) Serial.println("- failed to open file for appending:"+String(LOGFILE));
      }
       s="";
       if(n%10==0) {
       Bms.readBms();
