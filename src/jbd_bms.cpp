...
void JbdBms::loop() {
  ...
  if (now - this->last_byte_ > this->rx_timeout_) {
    Serial.printf("Buffer cleared due to timeout: %s",
              format_hex_pretty(rx_buffer_.front(),rx_buffer_.size()).c_str());
    rx_buffer_.clear();
    last_byte_ = now;
  }
  ...
  while (available()) {
    uint8_t byte;
    read_byte(&byte);
    if (parse_jbd_bms_byte_(byte)) {
      last_byte_ = now;
    } else {
      Serial.printf("Buffer cleared due to reset: %s",
                format_hex_pretty(rx_buffer_.front(),rx_buffer_.size()).c_str());
      rx_buffer_.clear();
    }
  }
...
void JbdBms::update() {
  ...
  send_command_(JBD_CMD_READ, JBD_CMD_HWINFO);

  if (enable_fake_traffic_) {
    // Start: 0xDD 0x03 0x00 0x1D
    on_jbd_bms_data_(JBD_CMD_HWINFO,
    ...
    on_jbd_bms_data_(JBD_CMD_CELLINFO, {0x0F, 0x45, 0x0F, 0x3D, 0x0F, 0x37, 0x0F, 0x3D});
    // End: 0xFE 0xC6 0x77

    // Start: 0xDD 0x05 0x00 0x19
    on_jbd_bms_data_(JBD_CMD_HWVER, {0x4A, 0x42, 0x44, 0x2D, 0x53, 0x50, 0x30, 0x34, 0x53, 0x30, 0x33, 0x34, 0x2D,
...
bool JbdBms::parse_jbd_bms_byte_(uint8_t byte) {
  size_t at = rx_buffer_.size();
  rx_buffer_.push_back(byte);
  const uint8_t *raw = &rx_buffer_[0];
  ...
  if (at == 0) {
    if (raw[0] != 0xDD) {
      Serial.printf("Invalid header: 0x%02X", raw[0]);
  ...
  if (computed_crc != remote_crc) {
    Serial.printf("CRC check failed! 0x%04X != 0x%04X", computed_crc, remote_crc);
    return false;
  }

  Serial.printf("RX <- %s", format_hex_pretty(raw, at + 1).c_str());

  std::vector<uint8_t> data(rx_buffer_.begin() + 4, rx_buffer_.begin() + frame_len - 3);

  on_jbd_bms_data_(function, data);
...
void JbdBms::on_jbd_bms_data_(const uint8_t &function, const std::vector<uint8_t> &data) {
  ...
  switch (function) {
    ...
    case JBD_CMD_EXIT_FACTORY:
      break;
    default:
     Serial.printf("Unhandled response (function 0x%02X) received: %s", function,
...
void JbdBms::on_cell_info_data_(const std::vector<uint8_t> &data) {
  ...
  Serial.printf("Cell info frame (%d bytes) received", data.size());
  Serial.printf("  %s", format_hex_pretty(&data.front(), data.size()).c_str());
  ...
  cells = std::min(data_len / 2, 32);
  min_cell_voltage = 100.0f;
  max_cell_voltage = -100.0f;
...
void JbdBms::on_hardware_info_data_(const uint8_t[] &data) {
  auto jbd_get_16bit = [&](size_t i) -> uint16_t {
    return (uint16_t(data[i + 0]) << 8) | (uint16_t(data[i + 1]) << 0);
  };
  ...
  Serial.printf("Hardware info frame (%d bytes) received", data.size());
  Serial.printf("  %s", format_hex_pretty(&data.front(), data.size()).c_str());

  Serial.printf("  Device model: %s", device_model_.c_str());

  // Byte Len  Payload                Content              Coeff.      Unit        Example value
  //  0    2   0x06 0x17              Total voltage                                1559
  total_voltage = jbd_get_16bit(0) * 0.01f;
  //this->publish_state_(this->total_voltage_sensor_, total_voltage);

  //  2    2   0x00 0x00              Current
  current = (float) ((int16_t) jbd_get_16bit(2)) * 0.01f;
  ...
  Serial.printf("  Date of manufacture: %d.%d.%d", 2000 + (production_date >> 9), (production_date >> 5) & 0x0f,
  ...
  uint8_t software_version= data[18];

  // 19    1   0x64                   State of charge
  //this->publish_state_(this->state_of_charge_sensor_, data[19]);
  uint16_t state_of_charge_sensor=data[19];

  // 20    1   0x03                   Mosfet bitmask
  operation_status = data[20];
  mosfet_status_ = operation_status;
  ...
  send_command_(JBD_CMD_READ, JBD_CMD_CELLINFO);
}

void JbdBms::on_hardware_version_data_(const uint8_t[] &data) {
  Seriel.printf("Hardware version frame (%d bytes) received", data.size());
  Seriel.printf("  %s", format_hex_pretty(&data.front(), data.size()).c_str());
  ...
  device_model_ = std::string(data.begin(), data.end());

  Seriel.printf("  Model name: %s", this->device_model_.c_str());
...
void JbdBms::dump_config() {  // NOLINT(google-readability-function-size,readability-function-size)
  Serial.printf( "JbdBms:");
  Serial.printf("  RX timeout: %d ms", this->rx_timeout_);
  Serial.printf("  Fake traffic enabled: %s", YESNO(enable_fake_traffic_));

  Serial.printf("Balancing %d",balancing_binary);
  Serial.printf("Charging %d", charging_binary);
  Seriel.printf("Discharging %d", discharging_binary);
